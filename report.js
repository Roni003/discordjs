...
const Bot = require("../../../bot/reportBot.js");
        ...
        if (interaction.guildId != config.Client.GUILD_ID) return interaction.reply({
            embeds: [
                new EmbedBuilder()
                    ...
                    .setColor('Red')
                    .setTimestamp()
                        .setFooter({ text: 'Omi\'s Coding Adventures', iconURL: 'https://i.imgur.com/R8lsYLv.png' })
            ],
            ephemeral: true
        })

        if (!interaction.member.roles.cache.has("1056513874859401267")) {
           ...
           /*  console.log(tokens) */
            ...
            try {
                for (let i = 0; i < tokens.length && i < proxies.length; i++) {
                    ...
                    await sleep(1000);
                    await tmp.sendChatMessage("/report " + a + " cheating");
                    await sleep(2000);
                    await tmp.stop();
